 ...
 * иначе на странице будет старое/другое меню.
 */
(function () {
  ...
  function buildDD() {
    return DD_ITEMS.map(item => {
      ...
      return `<a href="${item.href}" class="an-dd-item"><svg viewBox="0 0 24 24">${item.icon}</svg>${item.label}${badgeSlot}</a>`;
    }).join('');
  }
...
<nav class="antviz-nav" id="antvizNav">
  ...
  (async () => {
    try {
      /* ВАЖНО: импортируем auth/db из того же firebase-config.js, что и
         весь остальной сайт (profile.html, support.html и т.д.).
         Раньше тут стоял отдельный getAuth() без аргументов — он либо
         падал, потому что Firebase App ещё не был инициализирован к
         этому моменту (nav.js подключается раньше скрипта страницы),
         либо ловил другой инстанс приложения. В обоих случаях
         onAuthStateChanged либо не срабатывал, либо терял сессию —
         профиль навсегда оставался в гостевом виде. */
      const { auth, db } = await import(`${b}firebase-config.js`);
        ...
        if (user) {
          ...
          if (avatarEl) {
            avatarEl.innerHTML = (user.photoURL
              ...
              : name[0].toUpperCase()) + '<span class="an-notify-dot" id="anNotifyDot"></span>';
          }
