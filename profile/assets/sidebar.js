...
(function () {
  ...
  const CSS = `
    ...
    .pg-bar{
      ...
      margin:0 -48px 8px; padding:16px 48px 14px;
      background:var(--card, var(--bg,#fff));
      border-bottom:1px solid transparent;
      border-radius:32px 32px 0 0;
      transition:border-color .15s;
    }
    .pg-bar.is-stuck{ border-bottom-color:var(--border, var(--stroke,#dfe3e8)); }
    .pg-bar-in{
      max-width:var(--cab-max); margin:0 auto;
      display:flex; align-items:center; justify-content:space-between; gap:16px;
    }
    .pg-bar-left{ display:flex; align-items:center; gap:10px; min-width:0; }
    .pg-crumb{
      display:flex; align-items:center; gap:10px; min-width:0;
      font-family:'Geologica','Inter','Arial',sans-serif; font-size:14px;
      color:var(--muted, var(--text-dim,#707a8a));
    }
    .pg-crumb i{ font-style:normal; opacity:.45; }
    .pg-crumb b{ font-weight:500; color:var(--text,#191b1e); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
    .pg-back{
      display:inline-flex; align-items:center; gap:8px;
      padding:9px 16px 9px 10px; border-radius:14px;
      background:var(--bg2, var(--card-elevated,#f2f4f7)); color:var(--text,#191b1e);
      font-family:'Geologica','Inter','Arial',sans-serif; font-size:14px; font-weight:500;
      text-decoration:none; border:1px solid var(--border, var(--stroke,#dfe3e8));
      transition:background .15s, border-color .15s;
    }
    .pg-back:hover{ border-color:var(--text-faint,#9a9da2); }
    .pg-back svg{ width:18px; height:18px; stroke:currentColor; stroke-width:2; fill:none; stroke-linecap:round; stroke-linejoin:round; }
    .pg-back-title{
      font-family:'Geologica','Inter','Arial',sans-serif; font-size:14px; color:var(--muted, var(--text-dim,#707a8a));
    ...
    .pg-search{
      ...
      padding:0 8px 0 14px; height:44px; border-radius:14px; cursor:pointer;
      background:var(--bg2, var(--card-elevated,#f2f4f7)); color:var(--muted, var(--text-dim,#707a8a));
      border:1px solid var(--border, var(--stroke,#dfe3e8));
      font-family:'Geologica','Inter','Arial',sans-serif; font-size:14px; text-align:left;
      transition:border-color .15s, color .15s;
    }
    .pg-search:hover{ border-color:var(--text-faint,#9a9da2); color:var(--text,#191b1e); }
    .pg-search svg{ width:17px; height:17px; stroke:currentColor; stroke-width:2; fill:none; stroke-linecap:round; stroke-linejoin:round; flex-shrink:0; }
    .pg-search span{ flex:1; }
    .pg-search kbd{
      font-family:inherit; font-size:12px; padding:4px 8px; border-radius:8px;
      background:var(--card, #fff); border:1px solid var(--border, var(--stroke,#dfe3e8)); color:var(--muted, var(--text-dim,#707a8a));
    }
    .pg-tool{
      position:relative; width:44px; height:44px; border-radius:14px; flex-shrink:0;
      display:flex; align-items:center; justify-content:center; cursor:pointer;
      background:var(--bg2, var(--card-elevated,#f2f4f7)); color:var(--muted, var(--text-dim,#707a8a));
      border:1px solid var(--border, var(--stroke,#dfe3e8)); text-decoration:none;
      transition:border-color .15s, color .15s;
    }
    .pg-tool:hover{ border-color:var(--text-faint,#9a9da2); color:var(--text,#191b1e); }
    ...
    @media (min-width:981px) and (max-width:1200px){
      ...
      .pg-search span, .pg-search kbd{ display:none; }
      .pg-crumb span, .pg-crumb i{ display:none; }
      .pg-crumb b{ font-size:15px; }
    }
    .pg-dot{
  ...
  const NAV_ITEMS = [
    ...
    { key:'payments',      href:b+'profile/payments',      icon:'<rect x="2.5" y="5" width="19" height="14" rx="3"/><path d="M2.5 10h19M6.5 15h3"/>', label:'Платежи' },
    { key:'sites',         href:b+'profile/sites',         icon:'<circle cx="12" cy="12" r="9"/><path d="M3 12h18M12 3a14 14 0 010 18 14 14 0 010-18z"/>', label:'Мои проекты' },
  ...
  function init() {
    ...
    if (contentEl) {
      ...
      const leftHtml = backHref
        ? `<a class="pg-back" href="${backHref}"><svg viewBox="0 0 24 24"><path d="m15 18-6-6 6-6"/></svg>${backLabel}</a>${crumbTitle ? `<span class="pg-back-title" id="pgBackTitle">${crumbTitle}</span>` : ''}`
        : `<div class="pg-crumb"><span>Личный кабинет</span><i>/</i><b>${crumbTitle}</b></div>`;
      const bar = document.createElement('div');
      bar.className = 'pg-bar';
      bar.id = 'pgBar';
      bar.innerHTML = `<div class="pg-bar-in">
        <div class="pg-bar-left">${leftHtml}</div>
        <div class="pg-bar-right">
          <button type="button" class="pg-search" data-open-palette aria-label="Поиск по кабинету">
            <svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            <span>Поиск по кабинету</span><kbd>${isMac ? '⌘ K' : 'Ctrl K'}</kbd>
          </button>
          <a class="pg-tool" href="${b || '/'}" title="На antviz.ru" aria-label="На antviz.ru">
            <svg viewBox="0 0 24 24"><path d="M3 10.5 12 3l9 7.5"/><path d="M5 9.5V20a1 1 0 0 0 1 1h4v-6h4v6h4a1 1 0 0 0 1-1V9.5"/></svg>
          </a>
          ...
          <a class="pg-tool" href="${b}profile/notifications" title="Уведомления" aria-label="Уведомления">
            <svg viewBox="0 0 24 24"><path d="M6 8a6 6 0 0 1 12 0c0 5 2 6 2 6H4s2-1 2-6"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
            <span class="pg-dot" data-notif-dot></span>
          </a>
        </div>
      </div>`;
    ...
    if (!window.__antvizPaletteLoading) {
      ...
      ps.src = scriptBase + 'palette.js?v=1';
      ps.defer = true;
  ...
  async function watchBadges(user) {
    ...
    try {
      ...
      if (notifResp.ok) {
        ...
        setBadge('notif', unreadNotifs, null);
        document.querySelectorAll('[data-notif-dot], #notifDot').forEach(el => { el.style.display = unreadNotifs ? 'block' : 'none'; });
      }
      if (ordersResp.ok) {
